...
def evaluate(circuit, g_tables, pbits_out, a_inputs, b_inputs):
    ...
    for gate in sorted(gates, key=lambda g: g["id"]):
        ...
        print(f"Evaluating gate {gate_id} with wire inputs {gate_in}")
        ...
        if msg:
            ...
            if gate_id in pbits_out:
                # Calculate the result for this gate's output
                result_bit = wire_inputs[gate_id][1] ^ pbits_out[gate_id]
                print(f"\tResult bit for gate {gate_id}: {result_bit}\n")
    ...
    for out in wire_outputs:
        ...
        print(f"Output wire {out}: {evaluation[out]}")
